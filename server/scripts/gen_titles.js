...
function generate_titles() {
	...
	Promise.reduce(wikis, function(unused, w) {
		...
		if (isTalk) {
			...
			commands = [
				"zcat dumps/" + wiki.replace(/-/g, '_') + "-latest-all-titles-in-ns0.gz | shuf | head -" + n + " > " + file,
				"head -2 " + file,
		...
		return execP(commands.join("; ")).then(function(out) {
			...
			for (let i = 0; i < titles.length; i++) {
				out.push('INSERT IGNORE INTO pages(title, prefix) VALUES("' + titles[i].replace(/"/g, '\\"') + '", "' + wikiWithNS + '");');
			}
